...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        Encoding = 'utf8'
        ModuleToBuild = 'PowerFIGlet'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/MischaBoender/PowerFIGlet'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Mischa Boender. All rights reserved."
        ModuleLicenseURI = 'https://github.com/MischaBoender/PowerFIGlet/LICENSE.md'
        ModuleTags = 'FIGlet' -split ','
        ModuleAuthor = 'Mischa Boender'
        ModuleDescription = 'FIGlet for PowerShell'
        ...
        # You should NOT enter this key here but rather manually enter it in the PowerFIGlet.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "PowerFIGlet.buildenvironment.json"
